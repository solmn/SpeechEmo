...
def build_model(X_train, Y_train, X_test, Y_test):
    ...
    inpu_s = (None, X_train[0].shape[1], X_train[0].shape[2], X_train[0].shape[3])
    print(inpu_s)
    model.add(TimeDistributed(Conv2D(32, kernel_size=(10,10), padding="same", activation="relu"), input_shape=inpu_s))
    model.add(TimeDistributed(MaxPooling2D(pool_size=(4, 4),  padding="same")))
    model.add(TimeDistributed(Dropout(0.2)))
    model.add(TimeDistributed(Conv2D(64, kernel_size=(10,10), activation="relu", padding="same")))
    model.add(TimeDistributed(MaxPooling2D(pool_size=(4, 4), padding="same")))
    model.add(TimeDistributed(Dropout(0.2)))
    model.add(TimeDistributed(Conv2D(64, kernel_size=(10,10), padding="same", activation="relu")))
    model.add(TimeDistributed(MaxPooling2D(pool_size=(4, 4), padding="same")))
    model.add(TimeDistributed(Dropout(0.2)))

    model.add(TimeDistributed(Conv2D(128, kernel_size=(10,10), padding="same", activation="relu")))
    model.add(TimeDistributed(MaxPooling2D(pool_size=(4, 4), padding="same")))
    model.add(TimeDistributed(Dropout(0.2)))
...
def main():
    ...
    with open("../../models/cnn/cnn_lstm__33.json", "w") as json_file:
        json_file.write(model_json)
    model.save_weights("../../models/cnn/cnn_lstm__33.h5")
    file = open("../../models/cnn/cnn_lstm___33.txt", "w")
